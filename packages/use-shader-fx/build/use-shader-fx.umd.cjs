...
}`;const fn=t=>{const s=u.useMemo(()=>new n.PlaneGeometry(2,2),[]),o=u.useMemo(()=>new n.ShaderMaterial({uniforms:{uTexture:{value:new n.Texture},uResolution:{value:new n.Vector2(0,0)},uBlurSize:{value:1}},vertexShader:cn,fragmentShader:vn}),[]);return U(t,s,o),o},te={texture:new n.Texture,blurSize:3,blurPower:5},dn=({size:t,dpr:s,samples:o=0})=>{const a=u.useMemo(()=>new n.Scene,[]),e=fn(a),l=R(t),v=u.useMemo(()=>({scene:a,camera:l,size:t,dpr:s,samples:o}),[a,l,t,s,o]),[f,r]=D(v),[c,m]=O(v),[p,d]=b(te);return[u.useCallback((h,y)=>{const{gl:M}=h;y&&d(y),i(e,"uTexture",p.texture),i(e,"uResolution",[p.texture.source.data.width,p.texture.source.data.height]),i(e,"uBlurSize",p.blurSize);let C=m(M);const T=p.blurPower;for(let P=0;P<T;P++)i(e,"uTexture",C),C=m(M);return r(M)},[r,m,e,d,p]),d,{scene:a,material:e,camera:l,renderTarget:f,output:f.texture}]};var mn=`varying vec2 vUv;
