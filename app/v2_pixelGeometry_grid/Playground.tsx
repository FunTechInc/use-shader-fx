...
const FxMaterialImpl2 = createFxMaterialImpl({
   uniforms: {
   ...
   fragmentShader: `
	...
	void main() {
		...
		vec2 u_gridCount = vec2(80.);
		vec2 u_target = vec2(450., 500.);
		vec3 u_fillColor = vec3(1.0, 1.0, 1.0);
		vec3 u_backgroundColor = vec3(0.0, 0.0, 0.0);
		vec3 u_gridColor = vec3(0., .5, 0.);
		...
		vec3 fillColor = (len >= 0.2) ? texture2D(spriteTexture, spriteUV).rgb : u_backgroundColor;
		// 4. マッピングに使うテクスチャのカラーをそのままレンダリング
		// vec3 fillColor = (len >= 0.8) ? texColor.rgb : u_backgroundColor;

		// 5. fill color
		// vec3 fillColor = (len >= 0.5) ? u_fillColor : u_backgroundColor;
...
const FxMaterialImpl = createFxMaterialImpl({
   ...
   fragmentShader: `
	...
	vec2 jitterNoise(vec2 uv, float t) {
		float jitterStrength = 0.002; // ジッター量
		float n1 = hash(uv + t);
	...
	vec3 rgbMaskStripe(vec2 uv) {
		float subPixelSize = 2.; // サブピクセルのサイズ
	...
	float scanline(vec2 uv) {
		float scanlineBaseBrightness = 0.1;
...
extend({ FxMaterialImpl, FxMaterialImpl2 });
...
export const Playground = () => {
   ...
   const material2 = useRef<any>();
   useEffect(() => {
      material.current?.updateResolution(size);
      material2.current?.updateResolution(size);
   }, [size]);
   ...
   useFrame((state) => {
      ...
      material2.current.uniforms.time.value = state.clock.getElapsedTime();
   });
